...
- Migration 061 is recorded in production and preview. It restores room and inventory deletion for approved partner owners only while keeping delegated managers blocked from deletion.
- Migration 060 remains repository/preview-only until a separate production approval. It prevents delegated managers from editing an active property or changing its publication state; delegated content edits are limited to properties that were already inactive. Do not deploy the manager application flow, send a manager invitation, or activate a hotel manager until production migration 060 is applied and verified.
...
3. Confirm migrations 055–059 and 061 remain recorded in production, apply and verify migration 060 under a separate approval, then—and only then—deploy the corresponding application changes or invite/activate a hotel manager under another approval.
