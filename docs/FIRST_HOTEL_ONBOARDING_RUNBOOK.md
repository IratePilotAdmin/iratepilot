...
- Migrations 060–061 are repository/preview-only until a separate production approval. Migration 060 prevents delegated managers from editing an active property or changing its publication state; delegated content edits are limited to properties that were already inactive. Migration 061 restores room and inventory deletion for approved partner owners only while keeping delegated managers blocked from deletion. Do not deploy the manager application flow, send a manager invitation, or activate a hotel manager until production migrations 060 then 061 are applied and verified in that order.
...
3. Confirm migrations 055–059 remain recorded in production, apply and verify migrations 060 then 061 in exact order under a separate approval, then—and only then—deploy the corresponding application changes or invite/activate a hotel manager under another approval.
