...
The native iRatePilot.com connector is a separate integration setup from ordinary PMS reservations. A saved connection is not automatically live. The current code branch has an ARI receiver route and a Booking.com development-only adapter. Booking.com reservation XML can be normalized and placed in a disabled-by-default, test-only encrypted inbox with duplicate detection, but this does not create a PMS reservation. The public capability contract reports Booking.com as development-only and Expedia, Agoda, Airbnb, and Google Hotel as not implemented.

The production domain has not exposed the connector API routes in the latest recorded live probe. The protected Preview has returned the capabilities contract once, but repeatable receiver access and a hosted database check remain outstanding. A read-only rehearsal-database preflight query is available in supabase/verification/20260925_native_ota_staging_preflight.sql; its result must be reviewed before any migration or deployment. Do not expect iRatePilot.com reservations to arrive in the PMS until the inbox is wired to an atomic PMS reservation/folio transaction and an end-to-end sandbox round trip is verified.
