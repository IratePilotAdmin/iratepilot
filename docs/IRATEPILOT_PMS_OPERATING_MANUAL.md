...
The native iRatePilot.com connector is a separate integration setup from ordinary PMS reservations. A saved connection is not automatically live. The current code branch has an ARI receiver route and a Booking.com development-only adapter. Booking.com reservation XML can be normalized and placed in a disabled-by-default, test-only encrypted inbox with duplicate detection, but this does not create a PMS reservation. The capability contract reports Booking.com as development-only and Expedia, Agoda, Airbnb, and Google Hotels as not implemented.

The Partner Center's **Hotel system connections → Channel distribution** panel reports capability status and the current missing requirements. It is informational: it does not create a provider connection, map a property, or enable traffic. The panel is available in the Vercel Preview at [Partner Center integrations](https://iratepilotadmin-eqlq49ev1-irate-pilot.vercel.app/partner/integrations) and is not yet deployed to production.

A fresh read-only production probe returned 404 for both `/api/ota/capabilities` and `/api/pms/ari`; production is not running these connector routes. A rehearsal-database preflight query is available in `supabase/verification/20260925_native_ota_staging_preflight.sql`; its migration-ledger result remains unobserved. Do not expect iRatePilot.com reservations to arrive in the PMS until the routes are deployed, the database binding and migration sequence are verified, the inbox is wired to an atomic PMS reservation/folio transaction, and an end-to-end sandbox round trip passes.
