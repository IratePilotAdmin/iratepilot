...
Historical local validation (September 24, 2026 18:18 UTC) passed lint, typecheck, production build, the flight launch-evidence audit, and 3,047 tests across 479 files at source commit `62e1c41`, with a test/evidence-only follow-up committed as `b4101f9`. At that time 12 native PMS migrations used the canonical 12-digit filenames. The current source branch has 13 registered native PMS migrations after adding the test-only encrypted external OTA inbox. The flight migration gate recognizes and excludes the complete current set from its flight apply plan. The new migration and staging path have focused local tests, but the full repository gate has not been rerun on the latest source. No hosted connector migration or production deployment has been performed. Receipt for the historical validation: `release/evidence/native-ota-current-validation-2026-09-24.json`.
...
## Rehearsal database recheck (September 25, 2026)

The signed-in Supabase dashboard now reports the existing `iratepilot-hotel-rehearsal-20260908` project as Healthy on Micro compute, with no migration history and a recent backup. Its public schema has 43 tables and contains one property, one room, one inventory row, and one partner; the partner-application queue contains two rows. It has no Red Roof property match and does not show the PMS reservation-outbox tables required by the native ARI receiver migration. This database is therefore not yet a verified connector staging baseline: applying only the receiver migration would be out of order and would fail its required integration gates. A read-only prerequisite query is available at `supabase/verification/20260925_native_ota_staging_preflight.sql`; its SQL syntax and detection of a compatible base schema with missing connector tables were checked in PGlite. No database changes were made. Reconcile and validate the OTA base migration sequence in an isolated rehearsal before applying the native receiver and setup migrations. Receipt: `release/evidence/native-ota-rehearsal-recheck-2026-09-25.json`.

## Rehearsal migration preflight execution (September 25, 2026 01:58 UTC)

Executed the expanded read-only SQL preflight in the signed-in rehearsal SQL Editor. Required base relations and columns passed for `properties`, `partners`, `rooms`, `inventory`, `bookings`, `profiles`, `revenue_daily_inputs`, and `revenue_recommendations`. Supabase auth tables/functions, `gen_random_uuid()`, and the required roles are present. Both revenue cross-property integrity checks returned zero. The required `irp_pms_outbox_connections`, `irp_pms_booking_versions`, and `irp_pms_outbox` relations are absent, as expected before connector migrations. This clears the inspected schema prerequisites, but does not verify backup restoration, Red Roof property/tenant ownership or authorization, the hosted Vercel Preview database binding, nor the reservation/ARI round trip. No DDL or row changes were run. The full ordered migration still must be applied and verified through Supabase migration tooling in an approved isolated target; keep all connector switches disabled until the sandbox round trip passes. The execution result is recorded in `release/evidence/native-ota-rehearsal-recheck-2026-09-25.json`.

## Live route and deployment recheck (September 25, 2026)

Read-only Vercel metadata confirms `www.iratepilot.com` and `iratepilot.com` remain aliased to production deployment `dpl_CgLUV9bxXnmnqvVF869nL3iCafR5` at commit `3d293b9e6a9c25450475e78775ee1c380e92d4aa`. The latest OTA connector code is on a separate READY Preview deployment (`dpl_6cgzrZVu4tZc7Q34dpT8ietwpznG`, commit `62e1c419cf49a5a7add41d4b30c4fff5c633f7cb`, target `null`). Public GET checks confirm `/api/health` returns 200 and `/api/pms/ari` plus `/api/ota/capabilities` return 404 on the production host. The site is reachable, but production does not yet run the connector code. No production deployment, migration, credential change, booking, or traffic activation was performed. The Preview remains protected by Vercel SSO; its app-level response, hosted schema, and Red Roof round trip are still unverified. Receipt: `release/evidence/native-ota-live-probe-2026-09-25.json`.

## Evidence-only preview update (September 25, 2026 02:07 UTC)

The expanded rehearsal preflight and test were committed as `104035acdae11be115e284632de75d0023be08c6` and its Vercel deployment reached READY at `dpl_Ekg5Nf85Hx41AQN1DfoMVcY188G3` (`target: null`). This commit changes verification artifacts only; production remains on its existing deployment. A fresh read-only probe again returned health 200 and both connector routes 404 on `www.iratepilot.com`. The protected Preview application routes were not opened or tested. A temporary Supabase CLI invocation could not read migration history because this workspace has no `SUPABASE_ACCESS_TOKEN`; no authentication or migration command was attempted. Database DDL, data, and connector traffic remain unchanged. Receipt: `release/evidence/native-ota-rehearsal-recheck-2026-09-25.json`.

The earlier local ordered migration rehearsal applied 12 native OTA/PMS migrations to an ephemeral PostgreSQL-compatible database. The current migration-chain test now includes the 13th, external-OTA inbox migration; focused tests confirm the full chain plus inbox staging behavior. The rehearsal verifies reservation/ARI connection setup, room/rate mapping persistence, aggregate baseline readiness, and that native capture, delivery, and ARI activation remain disabled until explicit reviewed sandbox operations. It then simulates a dated ARI update, creates a synthetic guest reservation, verifies the reservation outbox snapshot, and confirms a subsequent ARI update subtracts that undelivered booking from sellable inventory. The external-provider inbox stores an encrypted event but is not connected to this native outbox or a PMS booking transaction. These checks validate SQL behavior against the modeled base schema only; they do not substitute for applying the chain through Supabase migration tooling on the hosted isolated target or a real iRatePilot.com booking. Test: `tests/native-pms-migration-chain.test.ts`.

The read-only staging preflight now inspects the Supabase migration ledger when present and reports which of the current 13 native migration versions are applied. PGlite tests cover both a missing ledger and a partial ledger. The previously recorded SQL run verified base schema and platform prerequisites, but the hosted migration-ledger result remains unobserved for the rehearsal project. Test: `tests/native-ota-staging-preflight.test.ts`.

## Preview route probe (September 25, 2026 02:26 UTC)

Vercel confirms the latest protected Preview deployment `dpl_CN6QSeeFdQ6XDpJ2ZfkxhwsVx2ca` is READY at commit `5a40ea36d0619a0ac96ccc044056b216ba5e2819`. One authenticated fetch of `/api/ota/capabilities` returned HTTP 200 and the expected contract (`nativePmsAri.implementation: available`, `trafficEnabled: false`, no external OTA providers). A repeat request and `/api/pms/ari` redirected to Vercel SSO. Therefore this is evidence that the Preview route can serve the connector contract, but repeatable authenticated route acceptance, receiver runtime verification, hosted database binding, and a sandbox reservation round trip remain open. Production remains unchanged. Receipt: `release/evidence/native-ota-rehearsal-recheck-2026-09-25.json`.
