...
Executed the read-only SQL preflight in the signed-in rehearsal SQL Editor. Required base relations and columns passed for `properties`, `partners`, `rooms`, `inventory`, and `bookings`. The required `irp_pms_outbox_connections`, `irp_pms_booking_versions`, and `irp_pms_outbox` relations are all absent, as expected before connector migrations. This is stronger evidence that the rehearsal has the required minimal base shape, but it does not prove every dependency in the remaining migration chain (including profile snapshot fields, auth roles, or revenue tables), ownership/tenant mapping, backup restoration, or property authorization. No DDL or row changes were run. Before applying anything, verify the full ordered connector migration dependency set and confirm this rehearsal project is the approved disposable target; keep all connector switches disabled. The execution result is recorded in `release/evidence/native-ota-rehearsal-recheck-2026-09-25.json`.
