...
Latest production-baseline port validation (September 24, 2026 14:10 UTC): lint, typecheck, production build, flight launch-evidence audit, and all 3,038 tests across 477 files pass. Both connector API routes are included in the build. The 11 native PMS migrations now use the canonical 12-digit filenames required by the live migration tooling; the flight migration gate recognizes and excludes this exact set from its flight apply plan. This is a local production-based candidate only. The preview migration runner has not been used and no hosted migration or production deployment has been performed. Receipt: `release/evidence/native-ota-production-port-validation-2026-09-24.json`.

## Deployment binding recheck (September 24, 2026)

Vercel's production deployment remains `dpl_CgLUV9bxXnmnqvVF869nL3iCafR5`, branch `agent/flight-live-foundation-20260823`, commit `3d293b9e6a9c25450475e78775ee1c380e92d4aa`. Public GET requests to `www.iratepilot.com/api/ota/capabilities` and `/api/pms/ari` returned HTTP 404 at 14:09 UTC. A new isolated branch, `codex/native-ota-production-port`, is based on that production commit and retains the live site's flight, hotel, and account pages while adding the connector. It has not been pushed or deployed yet. Hosted migration and the Red Roof sandbox round trip remain unverified. Receipt: `release/evidence/native-ota-production-port-validation-2026-09-24.json`.

The earlier feature-branch Preview remains protected by Vercel SSO and is not the production-based candidate. Do not infer its routes or database state from this new branch's local checks.
...
Connection setup uses a service-role-only transactional RPC for reservation and ARI configuration. The PGlite regression forces the ARI save to fail and confirms reservation scope and setup audit receipt are rolled back. On the production-based port, all 3,038 tests across 477 files, lint, typecheck, production build, and the flight launch-evidence audit pass. Hosted preview migration, property publication/mapping, and a real reservation round trip remain unverified. Receipt: `release/evidence/native-ota-production-port-validation-2026-09-24.json`.
