...
The current branch also passes a local ordered migration rehearsal applying all 12 native OTA/PMS migrations to an ephemeral PostgreSQL-compatible database. It verifies reservation/ARI connection setup, room/rate mapping persistence, aggregate baseline readiness, and that capture, delivery, and ARI activation remain disabled until explicit reviewed sandbox operations. It then simulates a dated ARI update, creates a guest reservation, verifies the reservation outbox snapshot, and confirms a subsequent ARI update subtracts that undelivered booking from sellable inventory. This validates migration ordering and SQL behavior against the modeled base schema only; it does not substitute for applying the chain through Supabase migration tooling on the hosted isolated target or a real iRatePilot.com booking. Test: `tests/native-pms-migration-chain.test.ts`.

The read-only staging preflight now inspects the Supabase migration ledger when present and reports which of the 12 native migration versions are applied. PGlite tests cover both a missing ledger and a partial ledger. This query has not yet been run in the signed-in hosted SQL Editor, so the result remains unobserved for the rehearsal project. Test: `tests/native-ota-staging-preflight.test.ts`.
