...
The current branch also passes a local ordered migration rehearsal applying all 12 native OTA/PMS migrations to an ephemeral PostgreSQL-compatible database. It verifies reservation/ARI connection setup, room/rate mapping persistence, the aggregate baseline readiness function, and that capture, delivery, and ARI activation remain disabled. This validates migration ordering and SQL compatibility against the modeled base schema only; it does not substitute for applying the chain through Supabase migration tooling on the hosted isolated target. Test: `tests/native-pms-migration-chain.test.ts`.
