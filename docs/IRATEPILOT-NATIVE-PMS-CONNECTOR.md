...
Latest candidate validation (September 24, 2026): full `pnpm check` passed lint with no warnings, typecheck, 979 tests across 230 files, and a production build. Internal checkout confirmation and sign-out use Next client-router navigation. The candidate is committed and deployed to the protected Preview as `f63eeb19193980e9cb662dea69fa3e04263a4568`; the latest Preview is READY. Its routes remain behind Vercel SSO, and the hosted migration and reservation round trip are not yet verified. Receipt: `release/evidence/native-ota-candidate-validation-2026-09-24-v2.json`.
