...
- The connector code is on branch `codex/native-ari-capabilities-preview-20260923` and has a READY Vercel preview. Vercel sign-in protection is enabled on that preview, so it is not a public partner endpoint.
...
- The admin API now supports reading configuration/audit history, saving a connection disabled, and a per-connection PATCH enable/disable action. Enabling is rejected unless the global receiver flag is on; the database function independently requires an admin actor, an active approved property, the matching enabled PMS reservation connection, and active room mappings. There is not yet an operator-facing activation screen; do not enable a connection with a direct database edit.
...
5. Keep the global receiver flag off until the migration is applied, the activation API is deployed, and an operator-facing admin screen or reviewed admin procedure is available. The activation API verifies the approved property, matching enabled reservation connection and active room mappings, writes a configuration/activation audit receipt, and allows disablement even while the global flag is off.
...
7. Once the operator UI and release approvals are complete, enable only the tested connection through the admin PATCH workflow; global enablement follows property-scoped acceptance. Observe route errors, inventory drift, pending reservation delivery and duplicate/conflict counts. Disable globally first if authentication, mapping, capacity or data-integrity checks fail.
