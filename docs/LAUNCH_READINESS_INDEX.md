...
2. Approve the access, inventory, booking, payment, and capacity limits in `docs/PRIVATE_PILOT_GUARDRAILS.md`.
3. Confirm Vercel Preview environment values using `docs/ENVIRONMENT_READINESS.md`.
4. Back up Supabase and record the backup in `docs/RELEASE_EVIDENCE.md`.
5. Run `supabase/preflight_20260802.sql` and resolve every blocker.
6. Apply migrations `202608020001` through `202608020025` in filename order.
7. Run `supabase/verify_schema.sql` and `supabase/postflight_20260802.sql`.
8. Execute every critical flow in `docs/PREVIEW_SMOKE_TEST.md` against the exact release commit.
9. Complete `docs/RELEASE_EVIDENCE.md` with deployment, database, test, and defect evidence.
10. Apply the decision rules in `docs/GO_NO_GO.md`.
11. If approved, follow `docs/PRODUCTION_CUTOVER.md` exactly.
12. Monitor using `docs/POST_LAUNCH_MONITORING.md` and reconcile with `docs/DAILY_RECONCILIATION.md`.
13. Use `docs/INCIDENT_SEVERITY.md` and `docs/ROLLBACK_RUNBOOK.md` for any incident.
...
- `docs/PRIVATE_PILOT_GUARDRAILS.md`
...
- Approved pilot caps and invited-access rules
...
- Private-pilot caps and named access owners have not been approved.
