...
- [ ] Reconcile the local phased-diligence decision into the committed activation model and generic aggregator binding after separate approval; until then `providerDecisionRecorded` remains false, `selectedProviderId` remains null, activation stage 1 remains incomplete, all three tracks remain 0 of 10, both kill switches remain engaged, and the accepted Preview remains unchanged.
- [ ] Commit and publish the provider-decision review packet, separate decision record, and roadmap reconciliation after separate approval.
