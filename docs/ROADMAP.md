...
- [x] Locally harden the provider-path validator against nested hidden stage metadata, non-enumerable nested safety locks, accessor-backed state before invocation, and top-level or nested proxies; pass 33 focused tests, ESLint, TypeScript, 1,275 tests across 262 files, and the optimized 115-page Next.js build while preserving every fail-closed counter and authority boundary.
- [ ] After separate approval, commit and normally push only the two validator-hardening files and five reconciled Car Rentals documents; then, under a later separate approval, deploy the resulting commit only to isolated Preview, complete authenticated `/admin/cars` acceptance, and publish exact evidence. No provider action, Production promotion, or Production change is implied.
