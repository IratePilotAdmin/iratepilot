...
const rate = (basisPoints: number | string | null | undefined) => basisPoints == null ? null : `${Number(basisPoints) / 100}%`;
const schedule = (financial: NonNullable<Reservation["financial"]>) => {
  if (financial.fee_schedule_version === "hotel_partner_commission_13_reward_fee_3_v1") return "Stored schedule · 13% + 3%";
  const commissionRate = rate(financial.partner_commission_rate_bps);
  const rewardRate = rate(financial.reward_program_fee_rate_bps);
  return commissionRate && rewardRate ? `Stored schedule · ${commissionRate} + ${rewardRate}` : "Legacy recorded split · commission rate unknown";
};
...
export function PartnerReservations() {
  ...
  return <section className="mt-8 grid gap-6">
    <p className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950"><strong>Current schedule for new hotel bookings:</strong> 13% iRatePilot Group, LLC commission + 3% iRate Rewards Program contribution = 16% total hotel distribution cost. Historical reservations retain their stored schedule and amounts.</p>
    ...
    <div className="divide-y">{visibleItems.map((item) => <article key={item.id} className="grid gap-5 p-6 lg:grid-cols-[1fr_auto] lg:items-start">
      <div><strong>{item.properties?.name} — {item.rooms?.name}</strong><p className="mt-1 text-sm text-slate-500">{item.profiles?.full_name || "Traveler"} · {item.check_in} to {item.check_out} · {item.guests} guests</p><p className="mt-2 text-xs uppercase tracking-wider text-slate-500">{item.confirmation_code} · {getBookingStatusLabel(item.status)}</p>{item.cancellation_reason && <div className="mt-4 border-l-2 border-red-500 bg-red-50 p-3 text-sm text-red-900"><strong>Cancellation context</strong><p className="mt-1 leading-6">{item.cancellation_reason}</p></div>}{item.financial && <><dl className="mt-5 grid gap-2 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2 xl:grid-cols-5"><div><dt className="text-slate-500">Recorded room revenue</dt><dd className="font-semibold">{money(item.financial.gross_room_revenue)}</dd></div><div><dt className="text-slate-500">Partner commission{rate(item.financial.partner_commission_rate_bps) ? ` (${rate(item.financial.partner_commission_rate_bps)})` : ""}</dt><dd className="font-semibold">−{money(item.financial.partner_commission)}</dd></div><div><dt className="text-slate-500">iRate Rewards contribution{rate(item.financial.reward_program_fee_rate_bps) ? ` (${rate(item.financial.reward_program_fee_rate_bps)})` : ""}</dt><dd className="font-semibold">−{money(item.financial.reward_program_fee || 0)}</dd></div><div><dt className="text-slate-500">Total deductions</dt><dd className="font-semibold">−{money(Number(item.financial.partner_commission) + Number(item.financial.reward_program_fee || 0))}</dd></div><div><dt className="text-slate-500">Estimated hotel payout</dt><dd className="font-semibold">{money(item.financial.partner_net)}</dd></div></dl><p className="mt-2 text-xs text-slate-500">{schedule(item.financial)}</p></>}<p className="mt-3 text-xs text-slate-500">Traveler service fee: {money(item.fees)} · Traveler total: {money(item.total)}</p><TripStatusTimeline entries={item.booking_status_history || []} /></div>
