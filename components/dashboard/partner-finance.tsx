...
type Financial = {
  id: string; gross_room_revenue: number | string; partner_commission: number | string; reward_program_fee?: number | string | null;
  partner_commission_rate_bps?: number | string | null; reward_program_fee_rate_bps?: number | string | null; fee_schedule_version?: string | null; partner_net: number | string;
...
export function PartnerFinance() {
  ...
  const totals = useMemo(() => financials.filter((row) => row.status !== "void").reduce((sum, row) => ({
    gross: sum.gross + Number(row.gross_room_revenue),
    commission: sum.commission + Number(row.partner_commission),
    rewardProgramFee: sum.rewardProgramFee + Number(row.reward_program_fee || 0),
    totalHotelDeductions: sum.totalHotelDeductions + Number(row.partner_commission) + Number(row.reward_program_fee || 0),
    net: sum.net + Number(row.partner_net),
  }), { gross: 0, commission: 0, rewardProgramFee: 0, totalHotelDeductions: 0, net: 0 }), [financials]);
