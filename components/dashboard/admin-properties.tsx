...
export function AdminProperties() {
  ...
  return <section className="card mt-8 overflow-hidden"><div className="border-b p-6"><h2 className="text-xl font-semibold">Property review queue</h2><p className="mt-1 text-sm text-slate-500">Approval controls whether a property is eligible for traveler search.</p>{message && <p role="status" className="mt-3 text-sm">{message}</p>}</div><div className="divide-y">{properties.length === 0 && <p className="p-6 text-sm text-slate-500">No properties awaiting review.</p>}{properties.map((property) => { const partnerApproved = property.partners?.status === "approved"; const publishable = property.readiness.ready && partnerApproved; return <article key={property.id} className="grid gap-4 p-6 md:grid-cols-[1fr_auto] md:items-center"><div><strong>{property.name}</strong><p className="mt-1 text-sm text-slate-500">{property.star_rating}-star {property.type.replace("_", " ")} · {property.city}, {property.country} · {property.partners?.business_name || "Partner"}</p>{!partnerApproved && <p className="mt-2 text-xs text-amber-700">Cannot publish yet: partner approval required</p>}{partnerApproved && !property.readiness.ready && <p className="mt-2 text-xs text-amber-700">Cannot publish yet: {property.readiness.missing.join(", ")}</p>}</div><div className="flex gap-2"><button disabled={property.active || !publishable} onClick={() => decide(property.id, true)} className="btn-primary disabled:cursor-not-allowed disabled:opacity-50">{property.active ? "Published" : publishable ? "Approve & publish" : !partnerApproved ? "Partner pending" : "Incomplete"}</button>{property.active && <button onClick={() => decide(property.id, false)} className="btn-secondary">Pause</button>}</div></article>; })}</div></section>;
}
