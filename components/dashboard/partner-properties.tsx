...
type Property = { id: string; name: string; slug: string; type: string; star_rating: number; description?: string | null; city: string; region?: string | null; country: string; active: boolean; image_url?: string | null; amenities?: string[]; readiness: { ready: boolean; missing: string[] } };
...
export function PartnerProperties() {
  ...
  async function updateDetails(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const response = await fetch(`/api/partner/properties/${editingProperty.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "details",
          ...
          slug: form.get("slug"),
          type: form.get("type"),
          starRating: form.get("starRating"),
      ...
      setMessage("Listing details could not be saved. Please try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="mt-8 grid gap-8 xl:grid-cols-[1fr_420px]">
      <section className="card overflow-hidden">
        <div className="border-b p-6"><h2 className="text-xl font-semibold">Your properties</h2><p className="mt-1 text-sm text-slate-500">Listings stay private until administrator approval.</p>{message && <p role="status" className="mt-3 text-sm">{message}</p>}</div>
      ...
      <form key={editingProperty?.id || "no-property"} onSubmit={updateDetails} className="card grid gap-4 p-6">
        <div><h2 className="text-xl font-semibold">Edit listing details</h2><p className="mt-1 text-sm text-slate-500">Correct listing information before review. Published changes return to review.</p></div>
        <label className="text-sm font-medium">Property<select className="input mt-2" value={editingId} onChange={(event) => setEditingId(event.target.value)} required><option value="">Select property</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}</select></label>
        <label className="text-sm font-medium">Property name<input name="name" className="input mt-2" defaultValue={editingProperty?.name || ""} disabled={!editingProperty} required minLength={3} /></label>
        <label className="text-sm font-medium">Property URL slug<input name="slug" className="input mt-2" defaultValue={editingProperty?.slug || ""} disabled={!editingProperty} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required /></label>
        <div className="grid grid-cols-2 gap-3"><label className="text-sm font-medium">Type<select name="type" className="input mt-2" defaultValue={editingProperty?.type || "hotel"} disabled={!editingProperty}><option value="hotel">Hotel</option><option value="resort">Resort</option><option value="vacation_home">Vacation home</option></select></label><label className="text-sm font-medium">Rating<select name="starRating" className="input mt-2" defaultValue={String(editingProperty?.star_rating || 4)} disabled={!editingProperty}><option value="4">4 star</option><option value="5">5 star</option></select></label></div>
        <label className="text-sm font-medium">Description<textarea name="description" className="input mt-2 min-h-28" defaultValue={editingProperty?.description || ""} disabled={!editingProperty} minLength={30} required /></label>
        <div className="grid grid-cols-2 gap-3"><label className="text-sm font-medium">City<input name="city" className="input mt-2" defaultValue={editingProperty?.city || ""} disabled={!editingProperty} required /></label><label className="text-sm font-medium">State/region<input name="region" className="input mt-2" defaultValue={editingProperty?.region || ""} disabled={!editingProperty} /></label></div>
        <label className="text-sm font-medium">Country<input name="country" className="input mt-2" defaultValue={editingProperty?.country || ""} disabled={!editingProperty} required /></label>
        <button disabled={busy || !editingProperty} className="btn-primary">Save listing details</button>
      </form>
