...
begin
  ...
  if p_decision = 'approve' then
    ...
    v_points := case
      when v_tier = 'business' then floor(v_booking.subtotal)::integer * 3
      when v_tier = 'basic' then floor(v_booking.subtotal)::integer * 2
      else 0
    end;
    if v_points > 0 then
...
declare
  ...
  v_inventory_subtotal numeric(12,2);
...
begin
  ...
  into v_inventory_days, v_inventory_subtotal
  from public.inventory
  where room_id = p_room_id
    and stay_date >= p_check_in
    and stay_date < p_check_out
  ...
  v_discount_rate := case
    when v_tier = 'business' then 0.10
    when v_tier = 'basic' then 0.05
    else 0
  end;
  v_subtotal := round(v_inventory_subtotal * (1 - v_discount_rate), 2);
  v_service_fee := 0;
  v_total := v_subtotal;
  if round(v_total * 100)::integer <> p_amount_total_cents then
    raise exception 'Payment total does not match inventory and membership benefits';
  end if;
  ...
  v_commission := round(v_subtotal * 0.14, 2);
  insert into public.booking_financials (
  ...
  v_points := case
    when v_tier = 'business' then floor(v_subtotal)::integer * 3
    when v_tier = 'basic' then floor(v_subtotal)::integer * 2
    else 0
  end;
  if v_points > 0 then
    insert into public.reward_ledger (user_id, booking_id, points, description)
