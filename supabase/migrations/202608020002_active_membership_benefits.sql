...
begin
  ...
  if p_decision = 'approve' then
    ...
    select case when membership_status = 'active' then membership_tier else 'none' end
      into v_tier from public.profiles where id = v_booking.customer_id;
    v_points := case
    ...
    insert into public.notifications (user_id, title, body) values (
      ...
      'Your iRatePilot request ' || v_booking.confirmation_code ||
        ' was declined. No payment has been collected.'
    );
  else
...
begin
  ...
  if round(v_total * 100)::integer <> p_amount_total_cents then raise exception 'Payment total does not match inventory'; end if;
  ...
  if v_points > 0 then
    ...
    update public.profiles set reward_points = reward_points + v_points where id = p_customer_id;
  end if;
...
revoke all on function public.complete_paid_test_booking(text,uuid,uuid,uuid,date,date,integer,text,integer) from public;
grant execute on function public.complete_paid_test_booking(text,uuid,uuid,uuid,date,date,integer,text,integer) to service_role;
