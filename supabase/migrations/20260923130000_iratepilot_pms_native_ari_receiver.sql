...
create table public.irp_pms_native_ari_activation_events (
  ...
  created_at timestamptz not null default now(),
  check ((enabled and evidence_reference is not null) or (not enabled and evidence_reference is null)),
  check (evidence_reference is null or (length(trim(evidence_reference)) between 5 and 160
    and evidence_reference = trim(evidence_reference)
    and evidence_reference ~ '^[A-Za-z0-9][A-Za-z0-9._:/ -]{4,159}$'))
);
...
alter table public.irp_pms_native_ari_activation_events enable row level security;
...
revoke all on public.irp_pms_native_ari_connections, public.irp_pms_native_ari_mappings,
  public.irp_pms_native_ari_activation_events, public.irp_pms_native_ari_events from public, anon, authenticated;
...
grant select, insert on public.irp_pms_native_ari_activation_events to service_role;
...
create function public.irp_pms_set_native_ari_enabled(
  p_request_id uuid,
  p_connection text,
  p_enabled boolean,
  p_actor uuid,
...
declare
  c public.irp_pms_native_ari_connections%rowtype;
  prior public.irp_pms_native_ari_activation_events%rowtype;
  mapping_count integer;
begin
  if p_request_id is null or p_connection is null or p_connection !~ '^[A-Za-z0-9_-]{1,80}$'
     or p_enabled is null or p_actor is null
     or (p_enabled and (p_evidence_reference is null
       or length(trim(p_evidence_reference)) not between 5 and 160
       or p_evidence_reference <> trim(p_evidence_reference)
       or p_evidence_reference !~ '^[A-Za-z0-9][A-Za-z0-9._:/ -]{4,159}$'))
     or (not p_enabled and p_evidence_reference is not null) then
  ...
  select * into prior from public.irp_pms_native_ari_activation_events
   where request_id = p_request_id for update;
  if found then
    if prior.connection_id = p_connection and prior.enabled = p_enabled
       and prior.actor_id = p_actor and prior.evidence_reference is not distinct from p_evidence_reference then
      return jsonb_build_object('connectionId', p_connection, 'enabled', c.enabled, 'outcome', 'duplicate');
    end if;
    raise exception 'Native ARI activation request conflict' using errcode = '23505';
  end if;
  ...
  if p_enabled then
    ...
    if not exists (
      select 1 from public.irp_pms_outbox_connections source
       where source.property_id = c.property_id and source.connection_id = p_connection
         and source.pms_property_id = c.pms_property_id and source.enabled
    ) then raise exception 'Matching PMS booking connector is not enabled' using errcode = '42501'; end if;
    select count(*) into mapping_count
      from public.irp_pms_native_ari_mappings m
      join public.rooms r on r.id = m.ota_room_id and r.property_id = c.property_id and r.active
     where m.connection_id = p_connection;
    if mapping_count < 1 or mapping_count <> (
      select count(*) from public.irp_pms_native_ari_mappings m where m.connection_id = p_connection
    ) then raise exception 'Native ARI mappings are missing or inactive' using errcode = '23503'; end if;
  end if;

  update public.irp_pms_native_ari_connections
     set enabled = p_enabled, updated_at = now()
   where connection_id = p_connection;
  insert into public.irp_pms_native_ari_activation_events(
    request_id, connection_id, actor_id, enabled, evidence_reference
  ) values (p_request_id, p_connection, p_actor, p_enabled, p_evidence_reference);
  return jsonb_build_object('connectionId', p_connection, 'enabled', p_enabled, 'outcome', 'updated');
end;
$$;
...
grant execute on function public.irp_pms_save_native_ari_connection(text,uuid,text,text,text,text,integer,jsonb)
  to service_role;
revoke all on function public.irp_pms_set_native_ari_enabled(uuid,text,boolean,uuid,text)
  from public, anon, authenticated;
grant execute on function public.irp_pms_set_native_ari_enabled(uuid,text,boolean,uuid,text)
  to service_role;
