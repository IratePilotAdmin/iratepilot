...
import { Accessibility, Building2, Car, CheckCircle2, Circle, ClipboardCheck, Fuel, GitCompareArrows, KeyRound, Network, ShieldAlert } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Car Rentals inventory normalization | iRatePilot Admin",
  description: "Read-only, provider-neutral car-rental inventory-normalization contracts and controls.",
};
  ...
  return (
    <DashboardShell title="Admin Console" items={adminNavigation}>
      <div className="mx-auto max-w-7xl">
        <p className="text-xs font-semibold uppercase tracking-[.18em] text-brand-700">Car Rentals · Phase 3</p>
        <div className="mt-2 grid gap-6 lg:grid-cols-[1fr_320px] lg:items-end">
          <div>
            <h1 className="text-3xl font-bold text-slate-950">Inventory-normalization workspace</h1>
            <p className="mt-3 max-w-3xl leading-7 text-slate-600">Review provider-neutral contracts for locations, opening hours, vehicle classes, capacity, transmission, powertrain, accessibility, and features. This read-only workspace validates contract design only and does not ingest supplier data or expose live inventory.</p>
          </div>
          <div className="rounded-2xl border border-amber-300 bg-amber-50 p-5">
            <div className="flex items-center gap-3 text-amber-950"><ShieldAlert className="h-5 w-5" /><strong>Contract only</strong></div>
            <p className="mt-2 text-sm leading-6 text-amber-900">{normalization.completedCount} of {normalization.totalCount} gates recorded. No supplier inventory is ingested, and every runtime authority remains disabled.</p>
          </div>
        </div>
        ...
        <section className="mt-12 rounded-2xl border border-red-300 bg-red-50 p-6">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-6 text-red-900">No supplier has been contacted or connected. No supplier inventory is ingested and no provider mapping exists. Supplier research or contact, accounts, credentials, external traffic, live inventory, rates, reservations, payments, database migrations, deployment, and Production changes remain outside Phase 3 and require separate approval.</p>
        </section>
      </div>
    </DashboardShell>
  );
