...
export const metadata: Metadata = {
  title: "Flight supplier selection plan",
  description: "Review the flight supplier selection and sandbox adapter plan while credentials, traffic, ticketing, payments, and Production remain disabled.",
};
...
export default function Page() {
  ...
  const locks = [
    ["Supplier selection", selection.selectionState === "not_selected" ? "Not selected" : "Selected"],
    ["Credentials", selection.credentialsAccepted ? "Accepted" : "Not accepted"],
    ["Sandbox adapter", selection.sandboxAdapterImplemented ? "Implemented" : "Not implemented"],
    ["Sandbox traffic", selection.sandboxTrafficAuthorized ? "Enabled" : "Disabled"],
    ["Production traffic", selection.productionTrafficAuthorized ? "Enabled" : "Disabled"],
    ["Ticketing", selection.ticketingAuthorized ? "Enabled" : "Disabled"],
    ["Flight payments", selection.paymentAuthorized ? "Enabled" : "Disabled"],
  ] as const;

  return (
    <DashboardShell title="Admin Console" items={adminNavigation}>
      <p className="text-xs font-semibold uppercase tracking-[.18em] text-brand-700">Flights · Phase 3 · Planning only</p>
      <h1 className="mt-2 text-3xl font-bold">Flight supplier selection plan</h1>
      <p className="mt-2 max-w-3xl text-slate-600">Prepare a neutral selection rubric and sandbox adapter contract without selecting a supplier, accepting credentials, or contacting an airline, distributor, consolidator, payment service, or ticketing system.</p>
      ...
      <section className="mt-8 grid gap-4 lg:grid-cols-[1fr_1.6fr]">
        <div className="rounded-2xl bg-slate-950 p-6 text-white">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-300">No supplier selected. Planning completion cannot accept credentials, implement an adapter, enable traffic, issue tickets, collect payment, or change Production.</p>
          <div className="mt-6 text-4xl font-bold">{selection.completedCount}/{selection.totalCount}</div>
          <p className="mt-1 text-xs uppercase tracking-wider text-slate-400">Phase 3 gates recorded complete</p>
        </div>
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Decision model</p><h2 className="mt-2 text-2xl font-bold">One-hundred-point selection rubric</h2></div><Scale className="h-7 w-7 text-slate-400" /></div>
