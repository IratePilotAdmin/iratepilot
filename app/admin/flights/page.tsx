...
export default function Page() {
  ...
  const rolloutConsumerLaunch = buildFlightRolloutConsumerLaunchActivation();
  ...
  return (
    <DashboardShell title="Admin Console" items={adminNavigation}>
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Following gate · payment and settlement</p><h2 className="mt-2 text-2xl font-bold">Flight payment and settlement readiness</h2></div><Scale className="h-7 w-7 text-slate-400" /></div>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Payment readiness is blocked by sandbox certification. Current state is {rolloutPaymentSettlement.completeRouteCount}/{rolloutPaymentSettlement.totalRoutes} route packets; no Stripe object, charge, provider settlement, refund, or money movement has occurred.</p>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Next gate · support and release</p><h2 className="mt-2 text-2xl font-bold">Controlled Preview and consumer-release readiness</h2></div><ClipboardCheck className="h-7 w-7 text-slate-400" /></div>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Support and release readiness is blocked by security and privacy review. Current state is {rolloutSupportRelease.completeRouteCount}/{rolloutSupportRelease.totalRoutes} route packets; no support channel, Preview release, consumer release, provider traffic, ticketing, or payment is authorized.</p>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Sandbox certification</p><h2 className="mt-2 text-2xl font-bold">Provider test-plan readiness</h2></div><TicketCheck className="h-7 w-7 text-slate-400" /></div>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Each approved candidate has a six-checkpoint sandbox certification plan. Current state is {connectorSandboxCertification.completeCertificationCount}/{connectorSandboxCertification.totalConnectors} complete; no sandbox request, ticket, payment, or provider traffic has occurred.</p>
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {connectorSandboxCertification.records.map((record) => (
            <article key={record.connectorId} className="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <section className="mt-10">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">The selected planning route preference is {connectorRouteDecision.primaryConnectorId} primary and {connectorRouteDecision.secondaryConnectorId} secondary. All {connectorRouting.totalCandidates} catalogued candidates remain operationally unassigned; current route-planning state is {connectorRouting.completeRoutingCount}/{connectorRouting.totalCandidates} complete and route enablement is off.</p>
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <p className="mt-2 max-w-3xl text-slate-600">Define the narrow purpose, accountable sender, independent approvals, immutable message, truthful disclosures, official channel, recipient-role validation, data-minimization, one-contact limit, expiry, revocation, stop, incident, receipt, closeout, and no-commitment controls that would be required before a future Duffel diligence contact could be considered. The recorded Duffel-primary and Sabre-secondary preference is documentation only and does not authorize contact, contracting, traffic, booking, ticketing, or payment. This page cannot identify or contact a recipient, draft or send a message, submit a form, place a call, create an account or case, receive evidence, accept terms, recommend or select a supplier, enable traffic, issue tickets, collect payment, or change Production.</p>
