...
export async function GET() {
  try {
    ...
    const { data, error, count } = await supabase.from("bookings")
      .select("id,confirmation_code,check_in,check_out,subtotal,taxes,fees,total,status,created_at,stripe_payment_intent_id,stripe_payment_mode,properties(name),rooms(name),booking_cancellation_requests(status,refund_amount)", { count: "exact" })
