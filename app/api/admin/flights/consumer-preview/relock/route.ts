...
export async function POST(request: Request) {
  ...
  if (idempotencyKey === null || !parsed?.success) {
    return privateNoStoreJson({ error: "The Consumer Preview relock request is invalid." }, 400);
  }

  try {
    const result = await relockFlightConsumerPreview(
    ...
    return privateNoStoreJson({
      error: status === 409
        ? "Consumer Preview relock did not pass the exact active-state check."
        : "Consumer Preview relock is unavailable.",
    }, status);
  }
}
