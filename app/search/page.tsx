...
export default async function SearchPage() {
  const { hotels, source } = await getMarketplaceHotels();
  return (
    <>
      <SiteHeader />
      <main>
        ...
        <section className="container-page py-10">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-6">
            <div><strong className="text-xl text-slate-950">{hotels.length} premium stays</strong><span className="ml-2 text-sm text-slate-500">{source === "database" ? "Approved marketplace inventory" : "Private demo inventory"}</span></div>
