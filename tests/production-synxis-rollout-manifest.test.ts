...
describe("SynXis production rollout manifest", () => {
  it("partitions every repository migration at the 038/039 boundary in exact order", () => {
    ...
    expect(manifest.pendingDeploymentVersions).toEqual(["202608150060", "202608150061"]);
  });

  it("records completed database rollout while preserving later launch gates", () => {
    expect(manifest.executionState).toBe(
      "migrations_001_059_applied_060_061_preview_pending_manager_acceptance_incomplete_synxis_traffic_disabled",
    );
    ...
    expect(manifest.requiredWriteGates.join(" ").toLowerCase()).toContain("migrations 060");
    expect(manifest.requiredWriteGates.join(" ").toLowerCase()).toContain("then 061");
    ...
    expect(manifest.stopConditions.join(" ")).toContain("before production migrations 060 and 061 are applied and verified");
  });
