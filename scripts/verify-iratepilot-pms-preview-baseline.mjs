...
await check('remote trigger set plus new outbox coexist',async()=>{const names=(await query("SELECT tgname FROM pg_trigger WHERE tgrelid='public.bookings'::regclass AND NOT tgisinternal ORDER BY tgname")).rows.map(r=>r.tgname);assert.deepEqual(names,['enforce_approved_partner_booking','irp_pms_booking_outbox','on_booking_status_changed'])});
await check('observed status-history trigger records changes',async()=>assert.equal(await value('SELECT count(*)::int FROM booking_status_history WHERE booking_id=$1',[booking]),2));
await check('inactive room rejects booking without an event',async()=>{const before=await value('SELECT count(*)::int FROM irp_pms_outbox');await query('UPDATE rooms SET active=false WHERE id=$1',[room]);await assert.rejects(insert,/active room/);assert.equal(await value('SELECT count(*)::int FROM irp_pms_outbox'),before);await query('UPDATE rooms SET active=true WHERE id=$1',[room])});
await check('invalid payment mode is rejected',async()=>{await assert.rejects(()=>query("UPDATE bookings SET stripe_payment_mode='invalid' WHERE id=$1",[booking]),/bookings_stripe_payment_mode_check/)});
...
console.log(JSON.stringify({passed:checks,failed:0,engine:'PGlite PostgreSQL',fixture:'Observed Preview booking triggers and payment constraint; minimal related-table fixtures; not a full clone'}));
