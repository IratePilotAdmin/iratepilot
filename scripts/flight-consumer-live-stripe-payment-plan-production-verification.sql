-- Flight Consumer Production dark-gate verification for migration 202608260103.
...
do $flight_stripe_production_dark_catalog$
declare
...
begin
  if current_database() <> 'postgres'
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: target identity or PostgreSQL major changed';
  end if;
  ...
  if to_regclass(
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: journal table identity is invalid';
  end if;
  ...
  if not (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: digest-only 36-column shape changed';
  end if;
  ...
  if exists (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: a constraint is not immediate/validated';
  end if;
  ...
  if not (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: exact five-index contract changed';
  end if;
  ...
  if not (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: forced RLS is not active';
  end if;
  ...
  if exists (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: policy or user trigger is present';
  end if;
  ...
  if exists (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: direct table privilege is present';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: recorder contract is invalid';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: recorder ACL is invalid';
  end if;
  ...
  if exists (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: shadow or mutation lifecycle RPC exists';
  end if;
  ...
  if (
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: journal was not empty before smoke';
  end if;

  if to_regclass('supabase_migrations.schema_migrations') is null then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: migration ledger is missing';
  end if;
  ...
  if v_ledger_latest_version is distinct from '202608220063' then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: migration ledger tip drifted';
  end if;
  ...
  if cardinality(v_object_only_versions_ledgered) <> 0 then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: object-only versions were unexpectedly ledgered';
  end if;
end;
$flight_stripe_production_dark_catalog$;

set local request.jwt.claims = '{"role":"service_role"}';
set local role service_role;

do $flight_stripe_production_dark_smoke$
declare
  v_created record;
  v_replay record;
  v_second record;
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: modern request.jwt.claims role is not honored';
  end if;
  ...
  if not v_direct_access_denied then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: service_role direct table read was allowed';
  end if;
  ...
  if v_created.decision <> 'created'
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: created/replay contract changed';
  end if;
  ...
  if not v_drift_refused then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: one-field drift was accepted';
  end if;
  ...
  if not v_malformed_refused then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: malformed evidence was accepted';
  end if;
  ...
  if v_second.decision <> 'created' then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: second independent plan was not created';
  end if;
  ...
  if not v_ambiguity_refused then
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: ambiguous identity was accepted';
  end if;
end;
$flight_stripe_production_dark_smoke$;

reset role;

do $flight_stripe_production_dark_zero_dispatch$
begin
  if not (
    select count(*) = 2
      and bool_and(
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: zero-dispatch row posture changed';
  end if;
end;
$flight_stripe_production_dark_zero_dispatch$;

rollback;

do $flight_stripe_production_dark_rollback_receipt$
begin
  if (
    select count(*)
      from public.flight_consumer_live_stripe_payment_intent_plans
    ...
    raise exception
      'FLIGHT_STRIPE_PRODUCTION_VERIFY_FAILED: synthetic rows survived rollback';
  end if;
end;
$flight_stripe_production_dark_rollback_receipt$;

select jsonb_build_object(
  'gate', 'flight_consumer_stripe_payment_plan_production_dark_verification',
  'result', 'PASS',
  ...
  'object_only_versions_099_101_102_103_ledger_entries_present', false,
  'migration_103_ledger_entry_present', false
) as flight_stripe_production_dark_verification_receipt;
