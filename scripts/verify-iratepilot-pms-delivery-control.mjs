...
await db.exec(await readFile(new URL('supabase/migrations/202609070141_iratepilot_pms_delivery_control.sql',root),'utf8'));
const pauseId=await value('SELECT gen_random_uuid()');
const control=(id,expected,on,ref='incident-review-123')=>query('SELECT * FROM irp_pms_set_delivery($1,$2,$3,$4,$5)',[property,id,expected,on,ref]);
await check('service role can pause without stopping capture',async()=>{await db.exec('SET ROLE service_role');await control(pauseId,true,false);await db.exec('RESET ROLE');assert.equal(await value('SELECT enabled AND NOT delivery_enabled FROM irp_pms_outbox_connections'),true)});
await check('pause blocks new claims including expired leases',async()=>{await db.exec("UPDATE irp_pms_outbox SET lease_until=now()-interval '1 second' WHERE state='leased'");assert.equal((await query('SELECT * FROM irp_pms_claim_event()')).rows.length,0)});
await check('booking changes continue queuing during pause',async()=>{await query('UPDATE bookings SET subtotal=460,total=460 WHERE id=$1',[booking]);assert.equal(await value('SELECT max(source_version)::int FROM irp_pms_outbox'),3)});
await check('mismatched expected state rejected',async()=>assert.rejects(()=>control('00000000-0000-0000-0000-000000000002',true,true),/state changed/));
await check('request reuse with changed parameters rejected',async()=>assert.rejects(()=>control(pauseId,false,true),/identity/));
const resumeId=await value('SELECT gen_random_uuid()');
await check('resume restores claims in version order',async()=>{await control(resumeId,false,true);const row=(await query('SELECT * FROM irp_pms_claim_event()')).rows[0];assert.equal(Number(row.source_version),2)});
await check('old pause retry returns receipt without pausing again',async()=>{await control(pauseId,true,false);assert.equal(await value('SELECT delivery_enabled FROM irp_pms_outbox_connections'),true);assert.equal(await value('SELECT count(*)::int FROM irp_pms_delivery_controls'),2)});
await check('client roles cannot control or read audit',async()=>{for(const role of ['anon','authenticated']){await db.exec('SET ROLE '+role);await assert.rejects(()=>control(pauseId,true,false));await assert.rejects(()=>query('SELECT * FROM irp_pms_delivery_controls'));await db.exec('RESET ROLE')}});
await check('service role cannot rewrite receipts',async()=>{await db.exec('SET ROLE service_role');await assert.rejects(()=>query("UPDATE irp_pms_delivery_controls SET review_reference='tampered'") );await db.exec('RESET ROLE')});
await check('resume cannot bypass initial review',async()=>{await db.exec('UPDATE irp_pms_outbox_connections SET delivery_enabled=false;UPDATE irp_pms_baseline_runs SET released_at=NULL');await assert.rejects(()=>control('00000000-0000-0000-0000-000000000003',false,true),/baseline/);await db.exec('UPDATE irp_pms_baseline_runs SET released_at=now()')});
await check('control rollback closes delivery but retains capture and receipts',async()=>{await db.exec(await readFile(new URL('supabase/rollbacks/202609070141_iratepilot_pms_delivery_control.rollback.sql',root),'utf8'));assert.equal(await value('SELECT enabled AND NOT delivery_enabled FROM irp_pms_outbox_connections'),true);assert.equal(await value('SELECT count(*)::int FROM irp_pms_delivery_controls'),2)});
