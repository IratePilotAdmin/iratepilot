import type {
  StandardPmsAvailabilityRequest, StandardPmsCancellation, StandardPmsCancelReservationRequest,
  StandardPmsCreateReservationRequest, StandardPmsMapper, StandardPmsOffer, StandardPmsReservation,
} from "../standard";

type JsonRecord = Record<string, unknown>;
function record(value: unknown, label: string): JsonRecord {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Guestline ${label} response is malformed`);
  return value as JsonRecord;
}
function text(value: unknown) { return typeof value === "string" || typeof value === "number" ? String(value) : undefined; }
function amount(value: unknown) {
  const result = Number(value); if (!Number.isFinite(result) || result < 0) throw new Error("Guestline offer amount is invalid"); return result;
}
function encode(value: JsonRecord) { return btoa(JSON.stringify(value)).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", ""); }
function decode(value: string) {
  try { const normalized = value.replaceAll("-", "+").replaceAll("_", "/");
    return record(JSON.parse(atob(normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "="))), "offer identifier");
  } catch { throw new Error("Guestline offer identifier is invalid"); }
}
function items(payload: unknown, label: string) {
  if (Array.isArray(payload)) return payload.map((item) => record(item, label));
  const source = record(payload, label); const list = source.items ?? source.results ?? source.offers ?? source.rooms;
  if (!Array.isArray(list)) throw new Error(`Guestline ${label} response is missing results`);
  return list.map((item) => record(item, label));
}

export type GuestlineBookingMapperConfig = { currency: string; bookingSourceCode?: string };
export class GuestlineBookingMapper implements StandardPmsMapper {
  constructor(private readonly config: GuestlineBookingMapperConfig) {
    if (!config.currency.trim()) throw new Error("Guestline currency is required");
  }
  ...
  availabilityResponse(payload: unknown, input: StandardPmsAvailabilityRequest): StandardPmsOffer[] {
    return items(payload, "availability").map((offer) => {
      ...
      if (!roomTypeCode || !ratePlanCode) throw new Error("Guestline offer is missing room or rate identifiers");
      const count = Number(offer.availableRooms ?? offer.availableCount ?? offer.inventory);
      const available = typeof offer.available === "boolean" ? offer.available : Number.isFinite(count) && count >= (input.rooms ?? 1);
  ...
  createReservationPayload(input: StandardPmsCreateReservationRequest) {
    const offer = decode(input.offerId); const roomTypeCode = text(offer.roomTypeCode); const ratePlanCode = text(offer.ratePlanCode);
    if (!roomTypeCode || !ratePlanCode) throw new Error("Guestline offer identifier is invalid");
    return { arrivalDate: input.arrivalDate, departureDate: input.departureDate, adults: input.adults,
  ...
  createReservationResponse(payload: unknown, input: StandardPmsCreateReservationRequest): StandardPmsReservation {
    ...
    const reservation = source.reservation && typeof source.reservation === "object" ? record(source.reservation, "reservation") : source;
    const reservationId = text(reservation.reservationId ?? reservation.id ?? reservation.confirmationNumber);
    if (!reservationId) throw new Error("Guestline reservation response is missing an ID");
    return { propertyCode: input.propertyCode, reservationId,
      ...
      externalReference: input.externalReference, status: text(reservation.status) ?? "CONFIRMED", raw: payload };
  }
  ...
  cancelReservationResponse(payload: unknown, input: StandardPmsCancelReservationRequest): StandardPmsCancellation {
    ...
    const confirmed = source.success === true || source.cancelled === true || (status ? ["cancelled", "canceled"].includes(status.toLowerCase()) : false);
    if (!confirmed) throw new Error("Guestline did not confirm the cancellation");
