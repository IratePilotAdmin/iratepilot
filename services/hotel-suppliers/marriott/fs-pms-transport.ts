...
export type MarriottFsPmsEndpoint = { path: string; method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE" };
export type MarriottFsPmsConfig = {
  baseUrl: string;
...
export type MarriottFsPmsFetch = (input: string | URL | Request, init?: RequestInit) => Promise<Response>;

export class MarriottFsPmsTransportError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "MarriottFsPmsTransportError";
  }
}

function validateConfig(config: MarriottFsPmsConfig) {
  ...
  if (!config.apiCredential?.trim() && !config.getApiCredential) {
    throw new Error("Marriott FS-PMS API credential or credential provider is required");
  }
  for (const operation of ["availability", "create_reservation", "cancel_reservation"] as const) {
    if (!config.endpoints[operation]?.path) throw new Error(`Marriott FS-PMS endpoint mapping is required for ${operation}`);
  }
  return baseUrl;
}
...
function payloadRecord(payload: unknown): Record<string, unknown> {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw new Error("Marriott FS-PMS payload must be an object");
  }
...
function reservationId(payload: Record<string, unknown>) {
  ...
  if ((typeof value !== "string" && typeof value !== "number") || !String(value).trim()) {
    throw new Error("Marriott FS-PMS cancellation requires a reservation ID");
  }
  return String(value);
}
...
export class MarriottFsPmsHttpTransport implements MarriottPmsTransport {
  private readonly baseUrl: URL;

  constructor(private readonly config: MarriottFsPmsConfig, private readonly fetcher: MarriottFsPmsFetch = fetch) {
  ...
  private async credential() {
    ...
    if (!value?.trim()) throw new Error("Marriott FS-PMS credential provider returned an empty value");
    return value;
  }

  async execute(request: MarriottTransportRequest): Promise<unknown> {
    if (request.provider !== "marriott-fs-pms") {
      throw new Error("Marriott FS-PMS transport only accepts marriott-fs-pms requests");
    }
    ...
    if (endpoint.origin !== this.baseUrl.origin) {
      throw new Error("Marriott FS-PMS endpoint must remain on the configured origin");
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new MarriottFsPmsTransportError(
          details.message || `Marriott FS-PMS request failed with status ${response.status}`,
          response.status, request.operation, request.requestId, details.code,
        );
      }
      return responsePayload;
    } catch (error) {
      if (error instanceof MarriottFsPmsTransportError) throw error;
      if (error instanceof Error && error.name === "AbortError") {
        throw new MarriottFsPmsTransportError("Marriott FS-PMS request timed out", 504, request.operation, request.requestId);
      }
      throw new MarriottFsPmsTransportError("Marriott FS-PMS request failed", 502, request.operation, request.requestId);
    } finally {
