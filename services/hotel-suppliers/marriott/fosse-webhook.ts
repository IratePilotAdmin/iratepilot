...
export type MarriottFosseEvent = { eventId: string; eventType: string; propertyCode: string; reservationId?: string; occurredAt?: string; data: Record<string, unknown> };
export function verifyMarriottFosseWebhook(body: string, signature: string | null, secret: string) {
...
export function parseMarriottFosseEvent(payload: unknown): MarriottFosseEvent {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) throw new Error("Marriott FOSSE event is malformed");
  ...
  if (!eventId || !eventType || !propertyCode) throw new Error("Marriott FOSSE event is missing required identifiers");
  return {
