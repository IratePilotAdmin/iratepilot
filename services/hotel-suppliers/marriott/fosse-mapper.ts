...
function record(value: unknown, label: string): JsonRecord {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Marriott FOSSE ${label} response is malformed`);
  return value as JsonRecord;
}
function text(value: unknown) { return typeof value === "string" || typeof value === "number" ? String(value) : undefined; }
function amount(value: unknown) {
  const result = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(result) || result < 0) throw new Error("Marriott FOSSE offer amount is invalid");
  return result;
}
function reservation(payload: unknown, input: MarriottGetReservationRequest | MarriottCreateReservationRequest): MarriottReservation {
  ...
  if (!reservationId) throw new Error("Marriott FOSSE reservation response is missing an ID");
  return {
...
export class MarriottFosseMapper implements MarriottPmsMapper {
  ...
  availabilityResponse(payload: unknown, input: MarriottAvailabilityRequest): MarriottOffer[] {
    ...
    return items.map((item) => {
      ...
      if (!offerId || !roomTypeCode || !ratePlanCode) throw new Error("Marriott FOSSE offer is missing required identifiers");
      return {
