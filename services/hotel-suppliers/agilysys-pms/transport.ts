...
export type AgilysysPmsEndpoint = {
  path: string;
  method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE";
};

export type AgilysysPmsConfig = {
  baseUrl: string;
  apiCredential?: string;
  getApiCredential?: () => Promise<string>;
  endpoints: Record<StandardPmsOperation, AgilysysPmsEndpoint>;
  credentialHeader?: string;
  credentialScheme?: string;
  timeoutMs?: number;
};

export type AgilysysPmsFetch = (input: string | URL | Request, init?: RequestInit) => Promise<Response>;

export class AgilysysPmsTransportError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "AgilysysPmsTransportError";
  }
}

function validateConfig(config: AgilysysPmsConfig) {
  const baseUrl = new URL(config.baseUrl);
  if (baseUrl.protocol !== "https:") throw new Error("Agilysys PMS base URL must use HTTPS");
  if (!config.apiCredential?.trim() && !config.getApiCredential) {
    throw new Error("Agilysys PMS API credential or credential provider is required");
  }
  for (const operation of ["availability", "create_reservation", "cancel_reservation"] as const) {
    if (!config.endpoints[operation]?.path) {
      throw new Error(`Agilysys PMS endpoint mapping is required for ${operation}`);
    }
  }
  return baseUrl;
}
...
function payloadRecord(payload: unknown): Record<string, unknown> {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw new Error("Agilysys PMS payload must be an object");
  }
...
function reservationId(payload: Record<string, unknown>) {
  ...
  if ((typeof value !== "string" && typeof value !== "number") || !String(value).trim()) {
    throw new Error("Agilysys PMS cancellation requires a reservation ID");
  }
  return String(value);
}
...
export class AgilysysPmsHttpTransport implements StandardPmsTransport {
  private readonly baseUrl: URL;

  constructor(
    private readonly config: AgilysysPmsConfig,
    private readonly fetcher: AgilysysPmsFetch = fetch,
  ) {
  ...
  private async credential() {
    ...
    if (!value?.trim()) throw new Error("Agilysys PMS credential provider returned an empty value");
    return value;
  }

  async execute(request: StandardPmsTransportRequest): Promise<unknown> {
    if (request.providerId !== "agilysys-pms") {
      throw new Error("Agilysys PMS transport only accepts agilysys-pms requests");
    }
    ...
    if (endpoint.origin !== this.baseUrl.origin) {
      throw new Error("Agilysys PMS endpoint must remain on the configured origin");
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new AgilysysPmsTransportError(
          details.message || `Agilysys PMS request failed with status ${response.status}`,
          response.status,
          request.operation,
          request.requestId,
          details.code,
      ...
      if (error instanceof AgilysysPmsTransportError) throw error;
      if (error instanceof Error && error.name === "AbortError") {
        throw new AgilysysPmsTransportError(
          "Agilysys PMS request timed out", 504, request.operation, request.requestId,
        );
      }
      throw new AgilysysPmsTransportError(
        "Agilysys PMS request failed", 502, request.operation, request.requestId,
      );
    } finally {
